...
- 按需 `Math Lab` 提供线性映射、复变映射与 domain coloring、Fourier/FFT、Laplace 数值积分和三维曲面；Markdown 公式可自动分类或手动选择后直接进入对应场景。重型渲染器默认不加载，并具备 Canvas 回退。见 [Math Lab](docs/math-lab.md) 与 [渲染兼容性](docs/math-renderer-compatibility.md)。
