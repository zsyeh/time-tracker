...
Math Lab 是 Learning OS 内由公式直接调用的按需数学可视化窗口。全局开关默认关闭，由管理员在 Django Admin 的站点策略中控制。关闭时 Markdown 渲染阶段不会生成公式入口，前端也会拒绝手动触发打开事件，因此工作区和重型引擎均不会加载。开启后，只有点击公式旁的 `↗` 才加载窗口外壳、自动分类结果以及对应引擎。
