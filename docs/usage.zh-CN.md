...
普通用户进入 `Settings` → `Invite codes`，每天（Asia/Shanghai 日期）可以生成一个只能使用一次的邀请码。管理员可以设置标签、1–100 次最大使用次数和可选到期时间。列表会明确显示已使用次数、剩余次数、最近使用时间，以及使用该邀请码注册的用户名。原始邀请码只显示一次，应立即复制。新用户打开 `/accounts/signup/`，填写用户名、密码和邀请码即可注册；短密码和纯数字密码允许使用，但更容易被猜到，建议首次登录后立即绑定 Passkey。
